...
class CollisionDetection {
  ...
  detectCircleCircle(dynamicShape, staticShape) {
    ...
    const maxRadiusSquared = Math.pow(
      dynamicShape.radius + staticShape.radius,
      2
    );

    if (distSquared < maxRadiusSquared) {
      // Normalenvektor (für Reflektion) berechnen
      const normal = dynamicShape.pos
        .clone()
        .subtract(staticShape.pos)
        .normalize();
      ...
      const vec = normal
        .clone()
        .multiplyScalar(staticShape.radius + dynamicShape.radius);
      const collisionPointDynamicShape = staticShape.pos.clone().add(vec);

      dynamicShape.hasCollided(
        staticShape.object,
        collisionPointDynamicShape,
        normal
      );
      staticShape.hasCollided(
        dynamicShape.object,
        collisionPointStaticShape,
        normal
      );
    }
  }
...
class CollisionShape {
  ...
  notifyCollisionListeners(otherObject, collisionPoint, normal) {
    this._collisionListeners.forEach((callback) =>
      callback(otherObject, collisionPoint, normal)
    );
  }
...
class CollisionLine extends CollisionShape {
...
class ComplexCollisionShape extends CollisionShape {
  ...
  updateBoundingBox() {
    const boundingBoxes = this.collisionShapes.map((shape) =>
